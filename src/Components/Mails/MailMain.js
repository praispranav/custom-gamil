...
const MailMain = (props) => {
  ...
  const Main = context.state.data
    .filter((item) => item.delete == false && item.spam == false)
    .map((item) => (
      <div>
        <Section
          state={state}
  ...
  const FilterSection = context.state.data
    .filter((item) => item.id === context.state.active)
    .map((item) => (
      <MailSection
        email={item.email}
        subject={item.subject}
  ...
  const Screen = context.state.data
  ...
  const Screen4 = context.state.data
    .filter((it) => it.spam === true)
    .map((item) => (
      <div>
        <Section
          keys={item.id}
  ...
  const Screen5 = context.state.data
    .filter((it) => it.labelled == true)
    .map((item) => (
      <div>
        <Section
          keys={item.id}
